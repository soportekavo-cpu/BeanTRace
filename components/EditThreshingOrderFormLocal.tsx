...
const EditThreshingOrderFormLocal: React.FC<EditThreshingOrderFormLocalProps> = ({ order, onCancel, onSaveSuccess }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [receiptsData, suppliersData, clientsData, orderReceiptsData, rendimientosData, reprocesosData, mezclasData] = await Promise.all([
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Apply new inventory and create receipts
            const applyPromises: any[] = [];
            for (const row of calculations.detailedRows) {
                const amount = Number(row.amountToThresh);
                if(amount <= 0) continue;

                let sourceNumber = availableReceipts.find(s => s.id === row.sourceId)?.recibo || '';
            ...
            const updatedOrderData: Partial<ThreshingOrder> = {
                clientId, clientName: clients.find(c=>c.id === clientId)?.name, description, lote, tipoPreparacion, pesoVendido: Number(pesoVendido) || 0,
                totalToThresh: calculations.totalToThresh, totalPrimeras: calculations.totalPrimeras, totalCatadura: calculations.totalCatadura,
            };
            await api.updateDocument<ThreshingOrder>('threshingOrders', order.id, updatedOrderData);

            onSaveSuccess();
        } catch (error) {
            console.error("Error updating local sale order:", error);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-card border border-border rounded-lg shadow-sm p-6">
                 <h3 className="text-lg font-semibold text-purple-600 mb-4 border-b pb-2">Asignar Insumos</h3>
                 <div className="overflow-x-auto">
                  ...
                  <button onClick={addInputRow} className="mt-4 flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-green-700 bg-green-100 rounded-md"><PlusIcon className="w-4 h-4" /> Añadir Insumo</button>
            </div>
